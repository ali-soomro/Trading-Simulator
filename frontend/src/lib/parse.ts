...
export function parseLine(line: string) {
  if (line.startsWith("BEST_BID")) {
    ...
    return { type: "BEST_BID", px: parseFloat(px), qty: parseInt(qty, 10) } as const;
  }
  if (line.startsWith("BEST_ASK")) {
    const [, px, , qty] = line.split(/\s+/);
    return { type: "BEST_ASK", px: parseFloat(px), qty: parseInt(qty, 10) } as const;
  }
  // Expect: "TRADE BUY 35 @ 50.21" (maybe with "against id ...")
  ...
  return { type: "OTHER", raw: line } as const;
}
